...
fn main() {
    ...
    let git_info = git_output
        .as_ref()
        .and_then(|output| str::from_utf8(&output.stdout).ok().map(str::trim));
    ...
    if let Some(git_info) = git_info {
        ...
        if git_info.contains(cargo_pkg_version) {
            ...
            git_describe.clone_from(&git_info.replace('g', ""));
        } else {
            git_describe = format!("v{cargo_pkg_version}-{git_info}");
        }
    }

    println!("cargo:rustc-env=MUDPUPPY_GIT_INFO={git_describe}");
}
