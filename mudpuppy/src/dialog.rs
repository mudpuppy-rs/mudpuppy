...
impl Dialog {
    ...
    pub fn increment_count(&mut self) {
        if let DialogKind::Notification {
            occurrence_count,
            message,
            ..
        } = &mut self.kind
...
impl DialogManager {
    ...
    pub fn show_warning(&mut self, message: String, timeout: Option<Duration>) {
        let dialog = Dialog {
            ...
            expires_at: Some(Instant::now() + timeout.unwrap_or(Duration::from_secs(10))),
            priority: DialogPriority::Normal,
        };
    ...
    pub fn show_info(&mut self, message: String, timeout: Option<Duration>) {
        let dialog = Dialog {
            ...
            expires_at: Some(Instant::now() + timeout.unwrap_or(Duration::from_secs(5))),
    ...
    pub fn handle_key(&mut self, key: &KeyEvent) -> (bool, Option<ConfirmAction>) {
