...
impl Handle {
    pub fn new(session: u32, mud: Mud, event_tx: UnboundedSender<Event>) -> Self {
        ...
        Self {
            session,
            task: Connection::spawn(session, mud, action_rx, event_tx),
            action_tx,
        }
    }

    pub fn send(&self, action: Action) -> Result<(), Error> {
        self.action_tx.send(action).map_err(Into::into)
    }

    pub fn send_line(&self, line: &str) -> Result<(), Error> {
        self.action_tx.send(Action::Send(telnet::codec::Item::Line(
