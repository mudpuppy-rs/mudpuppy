...
pub enum Item {
    ...
    ///
    /// For example, NOP (241), Go ahead (249). See [`telnet::command`] for helpful known
    /// command constants.
    IacCommand(u8),
...
impl TryFrom<Command> for Item {
    ...
    fn try_from(raw: Command) -> Result<Self, Self::Error> {
        Ok(match raw {
            ...
            _ => return Err(ErrorKind::Internal(format!("unexpected raw item: {raw:?}")).into()),
        })
    }
}
...
impl State {
    ...
    fn begin_subnegotiation(&mut self) -> Result<(), Error> {
        if !matches!(self, State::BufferingLine) {
            return Err(ErrorKind::Internal(format!("unexpected state: {self:?}")).into());
        }
