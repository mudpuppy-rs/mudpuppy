...
impl Trigger {
    ...
    pub(super) fn evaluate(
        ...
        // First, borrow mutable reference to trigger to perform a match test.
        // This will increment the hit_count stored in the trigger if it matches,
        // and yield any match groups.
        let groups = {
            let mut trigger = py_self.borrow_mut(py);
            if !trigger.enabled {
                trace!("ignoring disabled trigger {}", trigger.name);
                return Ok(());
            }
            trace!("evaluating trigger {}", trigger.name);
