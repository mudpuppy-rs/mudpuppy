...
use crate::session::{EchoState, InputLine, Mud, Session};
...
impl App {
    pub(super) fn new(args: cli::Args, config: Config) -> Self {
        Self {
    ...
    pub(super) async fn run(
        ...
        loop {
            ...
            select! {
                ...
                maybe_line = async {
                    // If we're in headless mode, wait on stdin reader. Otherwise,
                    // wait on a future that will never yield.
                    match &mut stdin_reader {
                    ...
                    match maybe_line {
                        Ok(Some(line)) => {
                            if let Err(e) = self.stdin(line).await {
                                error!("stdin error: {e}");
                            }
                        },
                        Ok(None) => {},
    ...
    async fn stdin(&mut self, line: String) -> Result<(), Error> {
        if let Some(line) = line.strip_prefix('/') {
            ...
            cmd.execute(self, remaining.to_string()).await?;
        } else if let Some(active_session) = self.active_session {
            debug!("sending stdin line: {line}");
            self.session_mut(active_session)?.send_line(
                InputLine {
                    sent: line,
                    original: None,
