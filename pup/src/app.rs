...
impl App {
    ...
    #[instrument(level = Level::TRACE, skip(self, py_rx), fields(active_session = ?self.data.active_session))]
        ...
        let result = loop {
            ...
            select! {
                ...
                Some((id, event)) = python_event_rx.recv() => {
                    self.data.session(id)?.event_handlers.session_event(&event)?;
                }
...
impl AppData {
    ...
    fn auto_connect(&mut self) -> Result<(), Error> {
